...
const Topics = ({ cat, toggleTagSelection, selectedCategory, suggestions }) => {
    ...
    return (
        <div>
            <div className={`cursor-pointer rounded-md border border-gray-200 text-center p-3 ${selectedCategory.includes(name) ? 'bg-green-500 bg-opacity-60 shadow-lg' : 'bg-blue-500 bg-opacity-60 shadow-sm'} text-xl text-white`} onClick={() => toggleTagSelection(name)}>
                <h2>{name}</h2>
            </div>
            {
